...
HTTP::HTTP (const std::shared_ptr<Config> &config,
    ...
    : config_ (config), log_ (log), buffer_ (buffer), parsedHttpRequest_ (),
...
HTTP::HTTP (const HTTP &http)
    : config_ (http.config_), log_ (http.log_), buffer_ (http.buffer_),
...
const std::string
HTTP::genHttpPostReqString (const std::string &body) const
{
...
const std::string
HTTP::genHttpOkResString (const std::string &body) const
{
