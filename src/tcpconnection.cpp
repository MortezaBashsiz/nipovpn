...
TCPConnection::TCPConnection (boost::asio::io_context &io_context,
    ...
    : socket_ (io_context), // Initialize the TCP socket with the I/O context
      ...
      timer_ (
          io_context), // Initialize the deadline timer with the I/O context
...
TCPConnection::doRead ()
{
  try
    ...
    {
      log_->write (std::string ("[TCPConnection doRead] [catch] ")
                       + error.what (),
...
void
TCPConnection::handleRead (const boost::system::error_code &error, size_t)
{
  try
    {
      if (error)
        {
          if (error == boost::asio::error::eof)
            {
              log_->write ("[TCPConnection handleRead] [EOF] Connection "
                           "closed by peer.",
                           Log::Level::TRACE);
            ...
            {
              log_->write (std::string ("[TCPConnection handleRead] ")
                               + error.message (),
                           Log::Level::TRACE);
            }
        }
              ...
              while (true)
                {
                  ...
                  if (read_error == boost::asio::error::eof)
                    {
                      log_->write ("[TCPConnection handleRead] [EOF] "
                    ...
                    {
                      log_->write (
                          std::string ("[TCPConnection handleRead] [error] ")
                              + read_error.message (),
                          Log::Level::ERROR);
    ...
    {
      log_->write (std::string ("[TCPConnection handleRead] [catch read] ")
                       + error.what (),
                   Log::Level::DEBUG);
  ...
  try
    {
      ...
      if (error)
        {
          log_->write (
              std::string ("[TCPConnection doWrite] [error] ")
                  + error.message (),
