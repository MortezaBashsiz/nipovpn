...
TCPClient::TCPClient (boost::asio::io_context &io_context,
    ...
    : config_ (config), log_ (log), io_context_ (io_context),
      socket_ (io_context), resolver_ (io_context), timer_ (io_context)
{
}
 ...
 */
void
TCPClient::writeBuffer (boost::asio::streambuf &buffer)
{
 ...
 */
void
TCPClient::doConnect (const std::string &dstIP, const unsigned short &dstPort)
{
  try
    {
      ...
      auto endpoint = resolver.resolve (dstIP.c_str (),
 ...
 */
void
TCPClient::doWrite (boost::asio::streambuf &buffer)
{
  try
    {
      moveStreambuf (buffer, writeBuffer_);
      ...
      if (error)
        {
          log_->write (std::string ("[TCPClient doWrite] [error] ")
                           + error.message (),
                       Log::Level::DEBUG);
  ...
  catch (std::exception &error)
    {
      ...
      log_->write (std::string ("[TCPClient doWrite] [catch] ")
                       + error.what (),
                   Log::Level::DEBUG);
...
TCPClient::doRead ()
{
  try
              ...
              while (true)
                {
                  ...
                  if (error == boost::asio::error::eof)
                    {
                      log_->write ("[TCPClient doRead] [EOF] Connection "
                                   "closed by peer.",
                                   Log::Level::TRACE);
                      socket_.close ();
                    ...
                    {
                      log_->write (std::string ("[TCPClient doRead] [error] ")
                                       + error.message (),
                                   Log::Level::ERROR);
                      socket_.close ();
                      return; // Exit after closing the socket
      ...
      if (readBuffer_.size () > 0)
        {
          try
          ...
          catch (std::exception &error)
            {
              ...
              log_->write (std::string ("[TCPClient doRead] [catch log] ")
                               + error.what (),
                           Log::Level::DEBUG);
            }
        }
  ...
  catch (std::exception &error)
    {
      ...
      log_->write (std::string ("[TCPClient doRead] [catch read] ")
                       + error.what (),
                   Log::Level::ERROR);
