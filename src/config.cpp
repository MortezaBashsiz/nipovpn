...
Config::Config (const RunMode &mode, const std::string &filePath)
    : runMode_ (mode), filePath_ (filePath),
      configYaml_ (YAML::LoadFile (filePath)), threads_ (0),
      listenIp_ ("127.0.0.1"), listenPort_ (0),
      general_ (
...
Config::Config (const Config::pointer &config)
    : runMode_ (config->runMode ()),
      ...
      general_ (config->general ()), log_ (config->log ()),
      server_ (config->server ()), agent_ (config->agent ())
{
}
 ...
 */
void
Config::threads (unsigned short threads)
{
  threads_ = threads;
}
 ...
 */
void
Config::listenIp (const std::string &ip)
{
  listenIp_ = ip;
}
 ...
 */
void
Config::listenPort (unsigned short port)
{
  listenPort_ = port;
}
