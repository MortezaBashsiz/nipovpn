...
ServerHandler::ServerHandler (boost::asio::streambuf &readBuffer,
    ...
    : config_ (config), log_ (log), client_ (client), readBuffer_ (readBuffer),
  ...
  if (request_->detectType ())
    {
      ...
      log_->write ("[ServerHandler handle] [Request From Agent] : "
                       + request_->toString (),
                   Log::Level::DEBUG);
      ...
      if (decryption.ok)
        {
          ...
          log_->write ("[ServerHandler handle] [Token Valid] : "
                           + request_->toString (),
                       Log::Level::DEBUG);
          ...
          if (request_->detectType ())
            {
              log_->write ("[ServerHandler handle] [Request] : "
                               + request_->toString (),
                           Log::Level::DEBUG);
                ...
                case HTTP::HttpType::connect:
                  {
                    ...
                    client_->doConnect (request_->dstIP (),
                                        request_->dstPort ());
                    ...
                    if (client_->socket ().is_open ())
                      ...
                      {
                        std::string message (
                            "HTTP/1.1 500 Connection failed\r\n\r\n");
                        os << message;
                      }
                    ...
                    if (request_->httpType () == HTTP::HttpType::http)
                      {
                        client_->doConnect (request_->dstIP (),
                                            request_->dstPort ());
                        log_->write (
                            "[CONNECT] [SRC " + clientConnStr_ + "] [DST "
                                + request_->dstIP () + ":"
                                + std::to_string (request_->dstPort ()) + "]",
                            Log::Level::INFO);
                      }
                    ...
                    if (client_->readBuffer ().size () > 0)
                      {
                        ...
                        if (encryption.ok)
                        ...
                        else
                          {
                            ...
                            log_->write ("[ServerHandler handle] [Encryption "
                                         "Failed] : [ "
                                             + decryption.message + "] ",
                                         Log::Level::DEBUG);
                            log_->write (
