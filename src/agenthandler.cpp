      ...
      if (request_->detectType ())
        {
          ...
          log_->write ("[AgentHandler handle] [Request] : "
                           + request_->toString (),
                       Log::Level::DEBUG);
          ...
          if (!client_->socket ().is_open ()
              ...
              || request_->httpType () == HTTP::HttpType::connect)
            {
              ...
              if (ec)
                {
                  log_->write (
                      std::string ("[AgentHandler handle] Connection error: ")
                          + ec.message (),
                      Log::Level::ERROR);
                  return;
                }
          ...
          if (client_->readBuffer ().size () > 0)
            {
              ...
              if (request_->httpType () != HTTP::HttpType::connect)
                  ...
                  if (response->parseHttpResp ())
                    {
                      ...
                      log_->write ("[AgentHandler handle] [Response] : "
                                       + response->restoString (),
                                   Log::Level::DEBUG);

                      // Decrypt the response body
                      BoolStr decryption{ false, std::string ("FAILED") };
                      decryption = aes256Decrypt (
                          decode64 (boost::lexical_cast<std::string> (
                      ...
                      if (decryption.ok)
                        {
                          ...
                          copyStringToStreambuf (decryption.message,
                                                 writeBuffer_);
                        }
                      else
                        {
              ...
              else
                {
                  ...
                  log_->write (
                      "[AgentHandler handle] [Response to connect] : \n"
                          + streambufToString (client_->readBuffer ()),
                      Log::Level::DEBUG);
