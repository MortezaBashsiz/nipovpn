            ...
            for (auto i = 0; i <= config_->general().repeatWait; i++) {
                while (true) {
                    ...
                    if (read_error == boost::asio::error::eof) {
                        ...
                        log_->write(std::string("[" + to_string(uuid_) + "] [TCPConnection handleReadAgent] [error] ") +
                                            read_error.message(),
                                    Log::Level::ERROR);
                        socketShutdown();
                        return;
...
void TCPConnection::doReadRest() {
    try {
        ...
        if (error) {
            if (error == boost::asio::error::eof) {
                log_->write(
                        "[" + to_string(uuid_) + "] [TCPConnection doReadRest] [EOF] Connection closed by peer.",
                        Log::Level::TRACE);
                socketShutdown();
                return;
            } else {
                log_->write(
                        std::string("[" + to_string(uuid_) + "] [TCPConnection doReadRest] ") + error.message(),
                        Log::Level::TRACE);
            }
        }
